...
if(plot.Extremes) {
  ...
  if (ana.time.res$time.res == ana.time.res$monthly) {
    ...
    PlotTowerExtremesContr(fino1.climobj, fname, extremes.thresh, ana.time.res)
    fname = paste0(outdir, "Fino2_MM-extremes_", res.switch, '_', fname_ext, ".pdf")
    PlotTowerExtremesContr(fino2.climobj, fname, extremes.thresh, ana.time.res)
    fname = paste0(outdir, "Cabauw_MM-extremes_", res.switch, '_', fname_ext, ".pdf")
    PlotTowerExtremesContr(cabauw.climobj, fname, extremes.thresh, ana.time.res)
    fname = paste0(outdir, "Lindenberg_MM-extremes_", res.switch, '_', fname_ext, ".pdf")
    PlotTowerExtremesContr(lind.climobj, fname, extremes.thresh, ana.time.res)
  } else if (ana.time.res$time.res == ana.time.res$hourly) {
